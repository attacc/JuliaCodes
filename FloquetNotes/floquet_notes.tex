...
\begin{equation}
\mathbf H_{mn}^F(k)=(n\Omega \mathbb{I} + Q\sigma_z) \delta_{mn} + i^{m-n} J_{m-n} cos\left [ k - \frac{(m-n)\pi}{2}  \right] \sigma_x
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The corresponding Floquet states are defined as the solution of:
\begin{equation}
    \sum_n \mathbf H^F_{mn} \chi_n^\alpha(k) = E_\alpha(k) \chi_n^\alpha(k)
\end{equation}
...
\be
\omega_\alpha= \vec \chi^\alpha(k)^\dagger \vec \psi_0(k) \\
\ee
where $\chi^\alpha_n(k)$ are defined as:
\be
\vec \chi^\alpha(k)=\sum_n \vec \chi^\alpha_n(k)
\ee
