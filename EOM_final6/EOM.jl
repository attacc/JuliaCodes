...
println(BN_orbitals.tau[1])
println(BN_orbitals.tau[2])

# Solve the Hamiltonian on a grid
...
Threads.@threads for ik in ProgressBar(1:k_grid.nk)
  ...
  #
  UdU=Grad_U(ik, k_grid, lattice, TB_sol, TB_gauge; orbitals=BN_orbitals, Hamiltonian=BN_Hamiltonian, deltaK=dk)
  #
...
if props.eval_curr
  j0_intra,j0_inter=current(rho0,k_grid,TB_sol,dyn_props,props)
...
let rho=copy(rho0)
  for it in ProgressBar(1:nsteps-1)
    ...
    #
    if props.eval_curr;  
       j_intra,j_inter=current(rho,k_grid,TB_sol,dyn_props,props)      
       j_intra-= j0_intra
       j_inter-= j0_inter
    ...
    if props.eval_pol;   
       pol=polarization(rho,k_grid,TB_sol,dyn_props) 
