...
\ee
These two Hamiltonian have the same eigenvalues (this is a similarity transformation) but their eigenvectors are related by:
\be
...
\eea
These result show that $ C_{n \kk}^j$ correspond to the cell periodic solution of the tight-binding, while $\tilde C_{n \kk}^j $ contains also the phase factor. These last one is the most common convention in tight-binding but it is not practical for the calculation of Berry phase related objects.
